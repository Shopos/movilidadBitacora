...
export async function agregarVehiculo(req:Request,res:Response){
    try{
        ...
        const id = await vehiculoModel.addVehiculo({patente,modelo,kms_actual,estado,tipo_vehiculo},usuario)
...
export async function editarVehiculo(req:Request,res:Response){
    try{
        ...
        const usuario = (req.usuario as any).correo
        const actualiza = await vehiculoModel.editVehiculo(id,{kms_actual,estado},usuario)
        if(!actualiza){
            return res.status(404).json({error:" vehiculo no encontrado"})
        }
